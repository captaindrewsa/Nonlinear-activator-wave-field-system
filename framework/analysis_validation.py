...
twospot_diagnostics.csv    optional component/separation/COM trajectory
...
def fit_tail(r: np.ndarray, psi_r: np.ndarray, r_min: float, r_max: float) -> Dict[str, Any]:
    ...
    mu is inferred from zero-crossing spacings (mu = pi/<Delta r>), and
...
def process_run(run_dir: str, args: argparse.Namespace) -> Optional[Dict[str, Any]]:
    ...
    for path in files:
        phi, psi, _v, t = load_snapshot(path)
        ...
        separation = periodic_distance(c0, c1, phi.shape, args.dx) if c0 and c1 else float("nan")
        records.append({
            "t": float(t), "phi_center": center, "phi_max": float(np.max(phi)), "phi_min": float(np.min(phi)),
            "psi_max_abs": float(np.max(np.abs(psi))), "n_components": len(components),
            "global_cx": gx, "global_cy": gy, "active_mass": mass,
            "c0_x": c0.get("cx", float("nan")), "c0_y": c0.get("cy", float("nan")),
            "c0_area": c0.get("area", float("nan")), "c0_r_eff": c0.get("r_eff", float("nan")),
            "c1_x": c1.get("cx", float("nan")), "c1_y": c1.get("cy", float("nan")),
            "c1_area": c1.get("area", float("nan")), "c1_r_eff": c1.get("r_eff", float("nan")),
    ...
    phi_max = np.asarray([r["phi_max"] for r in records], dtype=float)
    ...
    t0 = args.window_t0 if args.window_t0 is not None else float(t[0] + 0.6 * (t[-1] - t[0]))
    t1 = args.window_t1 if args.window_t1 is not None else float(t[-1])
    in_window = (t >= t0) & (t <= t1)
    tw, yw = t[in_window], phi_c[in_window]
    ...
    plot_timeseries(t, phi_c, phi_lo, phi_hi, figure_dir)
    if len(tw) >= 8:
    ...
    if final_phi is not None and final_psi is not None:
        ...
        if final_components:
            ...
            rphi, pphi = radial_profile(final_phi, core["cx"], core["cy"], args.dx)
            rpsi, ppsi = radial_profile(final_psi, core["cx"], core["cy"], args.dx)
            plot_radial(rphi, pphi, r"$\phi(r)$", "Final radial activator profile", "radial_profile_phi", figure_dir)
            plot_radial(rpsi, ppsi, r"$\psi(r)$", "Final radial inhibitor profile", "radial_profile_psi", figure_dir)
            rmax = float(np.max(rpsi))
            tail_result = fit_tail(rpsi, ppsi, args.tail_r_min if args.tail_r_min is not None else 0.25 * rmax, args.tail_r_max if args.tail_r_max is not None else 0.90 * rmax)
            tail_result["profile_time"] = float(t[-1])
            with open(Path(out_dir) / "tail_fit.json", "w", encoding="utf-8") as f:
                json.dump(tail_result, f, indent=2)

    sweep: List[Dict[str, Any]] = []
    if args.threshold_sweep:
        sweep = threshold_rows(t, phi_c, phi_lo, args, (t0, t1))
        if sweep:
    ...
    if args.two_spot:
        ...
        nx = final_phi.shape[0] if final_phi is not None else 1
        ny = final_phi.shape[1] if final_phi is not None else 1
        for row in records:
            if all(np.isfinite([row["global_cx"], row["global_cy"], start_x, start_y])):
                displacement = args.dx * math.hypot(periodic_delta(row["global_cx"], start_x, nx), periodic_delta(row["global_cy"], start_y, ny))
            else:
                displacement = float("nan")
            two_spot_rows.append({"t": row["t"], "n_components": row["n_components"], "separation": row["separation"], "global_cx": row["global_cx"], "global_cy": row["global_cy"], "com_displacement": displacement})
        with open(Path(out_dir) / "twospot_diagnostics.csv", "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=list(two_spot_rows[0].keys()))
            writer.writeheader()
            writer.writerows(two_spot_rows)
        plot_pair(t, np.asarray([q["separation"] for q in two_spot_rows]), np.asarray([q["com_displacement"] for q in two_spot_rows]), figure_dir)

    grid = {
        "nx": int(config_value(cfg, "nx", default=final_phi.shape[0] if final_phi is not None else 0)),
        "ny": int(config_value(cfg, "ny", default=final_phi.shape[1] if final_phi is not None else 0)),
        "dx": args.dx,
        "dt_config": config_value(cfg, "dt", default=None),
        "Lx": (final_phi.shape[0] * args.dx) if final_phi is not None else None,
        "Ly": (final_phi.shape[1] * args.dx) if final_phi is not None else None,
    }
    summary = {
        "run_id": run_id, "run_dir": run_dir, "analysis_name": args.analysis_name,
        "n_snapshots": len(records), "t_min": float(t[0]), "t_max": float(t[-1]),
        "grid": grid, "observation_window": {"t0": t0, "t1": t1},
        "A_pp": amp, "phi_max_window": float(np.max(yw)) if len(yw) else float("nan"),
        "frequency": fft["frequency"], "period": fft["period"], "r_fft": fft["r_fft"],
        "regime_label": regime,
        "classification_parameters": {"collapse_offset": args.collapse_offset, "amplitude_threshold": args.amplitude_threshold, "fft_prominence": args.fft_prominence},
        "tail_fit": tail_result, "threshold_sweep_rows": len(sweep), "two_spot_rows": len(two_spot_rows),
    }
    with open(Path(out_dir) / "summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)
    print(f"[ok] {run_id}: {regime}; A_pp={amp:.6g}; period={fft['period']:.6g}; {out_dir}")
    return summary
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--analysis-name", required=True, help="Results go to processed/<analysis-name>/")
...
def write_collection_manifest(out_dir: str, analysis_name: str, summaries: List[Dict[str, Any]]) -> None:
    ...
    with open(root / f"{stem}.csv", "w", newline="", encoding="utf-8") as f:
        keys = ["run_id", "run_dir", "n_snapshots", "regime_label", "A_pp", "period", "frequency", "r_fft"]
